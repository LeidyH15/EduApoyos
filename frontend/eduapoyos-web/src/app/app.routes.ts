    ...
    children: [
      {
        ...
        title: 'Panel del asesor | EduApoyos',
        loadComponent: () =>
          import(
            './features/asesor/pages/panel-asesor/panel-asesor'
          ).then((component) => component.PanelAsesor)
      },
      {
        path: 'estudiante/portal',
        title: 'Portal del estudiante | EduApoyos',
        loadComponent: () =>
          import(
            './features/estudiante/pages/portal-estudiante/portal-estudiante'
          ).then((component) => component.PortalEstudiante)
      },
      {
        path: 'solicitudes/nueva',
        title: 'Nueva solicitud | EduApoyos',
        loadComponent: () =>
          import(
            './features/solicitudes/pages/formulario-solicitud/formulario-solicitud'
          ).then((component) => component.FormularioSolicitud)
      },
      {
        path: 'solicitudes/:id',
        title: 'Detalle de solicitud | EduApoyos',
        loadComponent: () =>
          import(
            './features/solicitudes/pages/detalle-solicitud/detalle-solicitud'
          ).then((component) => component.DetalleSolicitud)
      }
    ]
  },
  {
    path: '**',
    title: 'Página no encontrada | EduApoyos',
    loadComponent: () =>
